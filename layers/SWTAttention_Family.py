...
def _alpha_to_logit(alpha):
    clipped_alpha = min(max(float(alpha), 1e-4), 1.0 - 1e-4)
    return log(clipped_alpha / (1.0 - clipped_alpha))


class GeomAttention(nn.Module):
    def __init__(self, mask_flag=False, factor=5, scale=None, attention_dropout=0.1, 
                 output_attention=False,
                 alpha=1., learnable_alpha=False,):
        ...
        if self.learnable_alpha:
            self.alpha_logit = nn.Parameter(
                torch.tensor(_alpha_to_logit(self.alpha), dtype=torch.float32)
            )
