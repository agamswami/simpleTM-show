...
if __name__ == '__main__':
    ...
    parser.add_argument('--hybrid_gate_mode', type=str, default='soft', choices=['soft', 'sparse', 'one_hot'],
                        help='Hybrid branch gate mode. soft preserves old behavior; sparse uses low-temperature softmax; one_hot uses straight-through hard branch selection.')
    parser.add_argument('--hybrid_branch_dropout', type=float, default=0.0,
                        help='Probability of dropping each Hybrid tokenizer branch during training.')
    parser.add_argument('--hybrid_gate_temperature', type=float, default=0.5,
                        help='Temperature used by sparse and one_hot Hybrid branch gates.')
    parser.add_argument('--hybrid_residual_branch', type=str, default='none', choices=['none', 'swt', 'fft', 'conv'],
                        help='Optional Hybrid residual fallback branch preserved as a base path.')
    parser.add_argument('--alpha', type=float, default=1, help='Initial/fixed weight of the wedge-product score in geometric attention')
    parser.add_argument('--learnable_alpha', action='store_true',
                        help='Make alpha in geometric attention learnable instead of fixed')
    ...
    if args.is_training:
        for ii in range(args.itr):
            ...
            setting = '_'.join(map(str, [
                ...
                args.hybrid_gate_mode,
                args.hybrid_branch_dropout,
                args.hybrid_gate_temperature,
                args.hybrid_residual_branch,
                args.m,
                args.alpha,
                args.learnable_alpha,
                args.l1_weight,
                args.learning_rate,
                args.lradj,
                args.batch_size,
